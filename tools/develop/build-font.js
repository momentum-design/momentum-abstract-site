...
const folderPath = path.resolve(my_path.scss,'_font');
const momentumPath = path.resolve(my_path.assets_momentum, 'font');
...
const generateScss = (file) => {
    Object.keys(file.content).forEach((key)=>{
        ...
        write.sync(path.join(folderPath, `${file.name}${_name}_mixins.scss`), _scss.join('\n\n'), { overwrite: true });
        write.sync(path.join(folderPath, `${file.name}${_name}.scss`), _css.join('\n'), { overwrite: true });
    });
};
